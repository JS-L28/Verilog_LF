...
module Display (input Ck, Reset, Start, SW0, TruePos, BC, BR, input [`BMP_LEN - 1 : 0] BMP, input done, Nomatch, alldone,
//input [`nodeLength-1: 0] Hashlen, 
input [`nodeLength-1:0] douta,
output reg [6:0] seg, output reg [7:0] digit, output reg [12:0] addrb, output reg R, G, B);
...
always @(posedge Ck or negedge Reset) 
 ...
 always @(posedge Ck or negedge Reset) begin
    ...
    else if (TruePos) begin G <= 1; end
    else if (Nomatch) begin R <= 1; end
    else if (alldone) begin B <= 1; end
